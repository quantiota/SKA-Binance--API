...
Signal logic:
  ...
  LONG:
    neutral→bull               (pending — waiting for pair confirmation)
    bull→neutral               (pair confirmed — OPEN LONG / IN_NEUTRAL)
    ...
    bear→neutral               (opposite pair confirmed — CLOSE LONG → OPEN SHORT)

  SHORT:
    neutral→bear               (pending — waiting for pair confirmation)
    bear→neutral               (pair confirmed — OPEN SHORT / IN_NEUTRAL)
    neutral→neutral × N        (neutral gap — stay IN_NEUTRAL)
    <first non-neutral>        (gap closes — READY)
    neutral→bear               (cycle repeats — WAIT_PAIR)
    neutral→bull               (opposite cycle opens — EXIT_WAIT)
    bull→neutral               (opposite pair confirmed — CLOSE SHORT → OPEN LONG)
...
MIN_NN_COUNT    = 3
MIN_TRADES      = 60    # wait for SKA convergence before trading
...
class SKATradingBot:
    ...
    async def process_signal(self, transition):
        ...
        # Do NOT react — keep position open through the spike; clear any pending entry
        ...
        if self.position is None:
            if name == 'neutral→bull':
                ...
                self._pending_short = False
                logging.info(f"--- neutral→bull seen — pending LONG confirmation | trade_id={trade_id}")

            elif name == 'neutral→bear':
                self._pending_short = True
                self._pending_long  = False
                logging.info(f"--- neutral→bear seen — pending SHORT confirmation | trade_id={trade_id}")

            elif name == 'bull→neutral' and self._pending_long:
                # Bull pair confirmed — BUY and open LONG
                if not self.dry_run:
                ...
                self.position = Position(
                    ...
                    entry_transition='bull→neutral', exit_state=IN_NEUTRAL
                )
                self.position.bull_pair_count = 1
                logging.info(
                    f">>> OPEN LONG (pair confirmed) @ {price:.6f} | P={p_str} | trade_id={trade_id}"
                )
                await self._log_event(trade_id, price, 'OPEN_LONG', IN_NEUTRAL, 'LONG', P=P)

            elif name == 'bear→neutral' and self._pending_short:
                # Bear pair confirmed — open SHORT (synthetic, no exchange order)
                self._pending_short = False
                self.position = Position(
                    side='SHORT', entry_price=price,
                    entry_trade_id=trade_id, entry_time=str(ts),
                    entry_transition='bear→neutral', exit_state=IN_NEUTRAL
                )
                self.position.bear_pair_count = 1
                logging.info(
                    f">>> OPEN SHORT (pair confirmed) @ {price:.6f} | P={p_str} | trade_id={trade_id}"
                )
                await self._log_event(trade_id, price, 'OPEN_SHORT', IN_NEUTRAL, 'SHORT', P=P)
            return
        ...
        if self.position.side == 'LONG':
            ...
            if self.position.exit_state == WAIT_PAIR:
                ...
                if name == 'bear→neutral' and P is not None and abs(P - P_X_NEUTRAL) <= TOL_CLOSE:
                    ...
                    # bear→neutral confirmed bear pair — immediately open SHORT (synthetic)
                    self.position = Position(
                        side='SHORT', entry_price=price,
                        entry_trade_id=trade_id, entry_time=str(ts),
                        entry_transition='bear→neutral', exit_state=IN_NEUTRAL
                    ...
                    self.position.bear_pair_count = 1
                    logging.info(
                        f">>> OPEN SHORT (re-entry) @ {price:.6f} | P={p_str} | trade_id={trade_id}"
                    )
                    await self._log_event(trade_id, price, 'OPEN_SHORT', IN_NEUTRAL, 'SHORT', P=P)
        ...
        # === SHORT POSITION ===
            ...
            if self.position.exit_state == WAIT_PAIR:
                ...
                if name == 'bull→neutral' and P is not None and abs(P - P_X_NEUTRAL) <= TOL_CLOSE:
                    ...
                    # bull→neutral confirmed bull pair — immediately open LONG (BUY already executed above)
                    self.position = Position(
                        side='LONG', entry_price=price,
                        entry_trade_id=trade_id, entry_time=str(ts),
                        entry_transition='bull→neutral', exit_state=IN_NEUTRAL
                    ...
                    self.position.bull_pair_count = 1
                    logging.info(
                        f">>> OPEN LONG (re-entry) @ {price:.6f} | P={p_str} | trade_id={trade_id}"
                    )
                    await self._log_event(trade_id, price, 'OPEN_LONG', IN_NEUTRAL, 'LONG', P=P)
                elif name == 'neutral→bear':
                    self.position.exit_state = WAIT_PAIR
